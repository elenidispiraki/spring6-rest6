...
public class TeacherRestController {
    ...
    public ResponseEntity<List<TeacherReadOnlyDTO>> getTeachers(
            @Nullable @RequestBody TeacherFilters filters, Principal principal)
    ...
    public ResponseEntity<Paginated<TeacherReadOnlyDTO>> getTeachersFilteredPaginated(
        ...
        try{
            if (filters == null) filters = TeacherFilters.builder().build();
            return ResponseEntity.ok(teacherService.getTeachersFilterdPaginated(filters));
        }catch(Exception e) {
            LOGGER.error("Error: Could not get teachers.", e);
            throw e;
        }
